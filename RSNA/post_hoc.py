...
from dataloader import Dataset_RSNA
import pandas as pd
...
def train_eval(dataset_name, dataset_class,bb_model_type, sel_model_type,depth,dim,num_patches,validation,load_bb_model):
  ...
  if dataset_name != 'rsna':
    ...
    num_classes = 2
    batch_size = 16
    train_datasize = len(pd.read_csv(os.path.join(csv_path,'train_labels.csv'))['Target'])
    valid_datasize = len(pd.read_csv(os.path.join(csv_path,'valid_labels.csv'))['Target'])
    test_datasize = len(pd.read_csv(os.path.join(csv_path,'test_labels.csv'))['Target'])

    train_dataset= Dataset_RSNA(data_file='train_labels.csv')
    trainloader = torch.utils.data.DataLoader(train_dataset, num_workers=0, batch_size=batch_size, shuffle=True, drop_last=True)

    val_dataset= Dataset_RSNA(data_file='valid_labels.csv')
    valloader = torch.utils.data.DataLoader(val_dataset, num_workers=0, batch_size=batch_size, shuffle=False, drop_last=True)

    test_dataset = Dataset_RSNA(data_file='test_labels.csv')
...
# mean and standard deviation of the metrics ph_acc and ICE.
  ...
  ################################################################################
    ...
    ## Initialize base blackbox model
    bb_checkpoint = torch.load(checkpoint_path+'/'+data_type+'_class'+str(num_classes)+'_model_'+'depth_'+str(depth)+'_dim_'+str(dim)+'.pt')
    bb_model = initialize_model(bb_model_type,num_classes=num_classes,input_dim=input_dim, channels=channels,patch_size=N,dim=dim,depth=depth,heads=8,mlp_dim=256,device=device)
    bb_model.load_state_dict(bb_checkpoint['model_state_dict'])
    #optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
...
if  __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name',  type=str,help="Dataset type: Options:[fmnist, mnist, cifar,rsna]", default= 'mnist')
